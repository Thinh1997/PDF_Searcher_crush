...
//#endif
	...
	*/
	...
	{
		e_continue, // PDFTron SDK will continue running and if necessary switch to demo mode when encountering an unresolvable connection error
		e_continue_unless_switching_to_demo, // PDFTron SDK will continue as long as it doesn't need to switch to demo mode in response to the connection error
											// If continuing without switching to demo mode is not possible, PDFTron SDK functions will throw exceptions.
		e_stop // PDFTron SDK will throw exceptions from subsequent usage if a connection error occurs
	};
	...
	*
	* @param mode Rules that PDFTron SDK will follow after a connection error.
	*/
